...
class Sonnet:
    ...
    def search_for(self, query: str):
        """Search this sonnet for the given query and return a SearchResult."""
...
class SearchResult:
    ...
    def print(self, idx: int, highlight: bool, total_docs: int):
        """Print this search result with optional highlighting."""
        title_line = SearchResult.ansi_highlight(self.title, self.title_spans) if highlight else self.title
        print(f"\n[{idx}/{total_docs}] {title_line}")

        for lm in self.line_matches:
            out_line = SearchResult.ansi_highlight(lm.text, lm.spans) if highlight else lm.text
            print(f"  [{lm.line_no:2}] {out_line}")
